...
dh_table = ((0, 0, 0, 0.1143),
            (theta1-sp.pi/2, sp.pi/2, 0, 0.11811),
            (theta2-sp.pi/2, 0, -0.1524, 0),
            (theta3+sp.pi/2, 0, 0.1524, 0),
            (theta4, 0, 0.0508, 0))
...
def calc_end_effector_transform() -> sp.Matrix:
    ...
    transM_5_0 = transM_1_0 * transM_2_1 * transM_3_2 * transM_4_3 * transM_5_4
    return transM_5_0

def main():
    """ Main function to run the code"""
    transM_5_0 = calc_end_effector_transform()
    print("the homogeneous transform for the end-effector w.r.t the base frame is: \n")
    sp.pprint(transM_5_0)
    print("-"*200, '\n')
    if ask_for_validation():  # ask user if they want to do forward position kinematics validation, if true, then continue
        ...
        x_e, y_e, z_e = transM_5_0_clc[:3, 3]
        ...
        meter_to_inch = 1/0.0254
        print(f"\nthe (x, y, z) coordinate of the end-effector is: {round(x_e*meter_to_inch,3), round(y_e*meter_to_inch,3),round(z_e*meter_to_inch,3)} inches")
